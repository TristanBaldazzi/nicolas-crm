                              ...
                              {(() => {
                                ...
                                return (
                                  <div className="text-right">
                                    ...
                                    <span className="text-sm font-bold text-green-700">TVA: {totalTVA.toFixed(2)} €</span>
                                  </div>
                                );
                              })()}
                            </div>
            ...
            <div className="space-y-4">
              {clientQuoteRequests.map((quoteRequest: any) => (
                ...
                >
                      ...
                      onClick={async () => {
                        try {
                          if (quoteRequest.isRead) {
                            await customQuotesApi.markAsUnread(quoteRequest._id);
                  ...
                  {/* Suggestions IA */}
                  {quoteRequest.aiSuggestions && quoteRequest.aiSuggestions.suggestedProducts && quoteRequest.aiSuggestions.suggestedProducts.length > 0 && (
                    <div className="mt-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {quoteRequest.user && !quoteRequest.autoCreatedCart && (
                          <button
                            onClick={async () => {
                              try {
                                await customQuotesApi.createCartFromSuggestions(quoteRequest._id);
                            ...
                            className="px-3 py-1.5 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg text-xs font-semibold hover:from-purple-700 hover:to-purple-800 transition-all"
                          >
                            Créer le panier
                          </button>
                        )}
                        {quoteRequest.autoCreatedCart && (
                          <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs font-semibold">
                            Panier créé
                          </span>
                        )}
                      ...
                      {quoteRequest.aiSuggestions.summary && (
                        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 mb-3">
                          <p className="text-xs text-purple-900">{quoteRequest.aiSuggestions.summary}</p>
                        </div>
                      )}

                      <div className="space-y-2">
                        {quoteRequest.aiSuggestions.suggestedProducts.map((suggestion: any, index: number) => (
                          <div
                            key={index}
                                ...
                                {suggestion.product && typeof suggestion.product === 'object' ? (
                                  <>
                                    <div className="flex items-center gap-2 mb-1">
                                      <h6 className="font-semibold text-gray-900 text-sm">{suggestion.product.name}</h6>
                                      <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs font-semibold">
                                        x{suggestion.quantity || 1}
                                      </span>
                                    </div>
                                  ...
                                  <>
                                    <h6 className="font-semibold text-gray-900 text-sm mb-1">Produit ID: {suggestion.product}</h6>
                  ...
                  {quoteRequest.autoCreatedCart && (
                    <div className="mt-4">
                      ...
                      {typeof quoteRequest.autoCreatedCart === 'object' ? (
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                          <Link
                            href={`/admin/paniers/${quoteRequest.autoCreatedCart._id}`}
                            className="inline-flex items-center gap-1 text-xs text-green-700 hover:text-green-800 font-semibold"
                          >
                            Voir le panier
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                          <Link
                            href={`/admin/paniers/${quoteRequest.autoCreatedCart}`}
                            className="inline-flex items-center gap-1 text-xs text-green-700 hover:text-green-800 font-semibold"
                          >
                            Voir le panier
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
